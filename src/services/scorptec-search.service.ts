...
export class ScorptecSearchService {
  ...
  async searchProduct(page: Page, productQuery: string): Promise<AmazonSearchResult[]> {
    try {
      ...
      const isAlreadyOnScorptec = currentUrl.includes(this.domain);

      if (!isAlreadyOnScorptec) {
        logger.info(`Visiting Scorptec homepage...`);
        ...
        // Try to set location to Dandenong
        try {
          const locationBtn = await page.evaluateHandle(() => {
            const els = Array.from(document.querySelectorAll('div, span, button, a'));
            // Look for the element containing "Delivery (AUS Wide)"
            return els.find(el => el.textContent?.trim().includes('Delivery (AUS Wide)') && el.children.length < 5);
          });
          if (locationBtn && locationBtn.asElement()) {
            await locationBtn.asElement()?.click();
            await randomDelay(1000, 2000);
            
            // Click Dandenong
            const dandenongOption = await page.evaluateHandle(() => {
              const els = Array.from(document.querySelectorAll('div, li, a, span'));
              return els.find(el => el.textContent?.includes('Dandenong') && !el.children.length);
            });
            if (dandenongOption && dandenongOption.asElement()) {
              await dandenongOption.asElement()?.click();
              await randomDelay(2000, 3000);
              logger.info("Location successfully set to Dandenong.");
            }
          }
        } catch (e) {
          logger.warn("Could not set location on Scorptec, proceeding anyway...");
        }
      }
      ...
      // Find search input
      const searchInputSelector = "input#sli_search_1, input[name='w']";
      ...
      try {
        ...
        await saveDebugSnapshot(page, "no_searchbox_scorptec");
      ...
      await page.waitForFunction(() => {
        return (
          !!document.querySelector('.product-list-item, .product-item, .card, [data-test="product-card"]') ||
          document.body.innerText.includes("We couldn't find any matches") ||
          document.body.innerText.includes("No results")
        );
      }, { timeout: 15000 }).catch(() => {
        logger.warn("Timeout waiting for Scorptec search results.");
      });
    ...
    const results = await page.evaluate((maxResults) => {
      ...
      logger.error(`Scorptec Search failed: ${(error as Error).message}`);
      return [];
    }
  }

  async selectVariantsAndGetPrice(page: Page, product: any): Promise<{price: number | null, cleanUrl: string}> {
    logger.info(`Selecting Scorptec variants for: ${product.productName}`);
    await randomDelay(2000, 3000);

    const specs = extractSpecs(product.productName);

    // Scorptec specific: Laptops often have different configurations
    // 1. Storage selection
    ...
    const price = await page.evaluate(() => {
      const priceEl = document.querySelector('.price, .product-price, .ticket-price');
      if (priceEl) {
        const match = priceEl.textContent?.replace(/[^0-9.]/g, "");
