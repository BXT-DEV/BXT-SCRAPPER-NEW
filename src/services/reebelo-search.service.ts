...
export class ReebeloSearchService {
  ...
  private async performSearch(
    ...
    const results = await page.evaluate((maxResults) => {
      ...
      // First pass: prefer links that already carry a skuId (phones, tablets).
      // Second pass (fallback): accept collection/product links without skuId —
      // needed for Watches and Laptops whose search-result cards omit the skuId.
      const withSkuId = links.filter((a) => {
      ...
      const withoutSkuId = links.filter((a) => {
        const href = a.getAttribute("href") || "";
        return (
          (href.includes("/collections/") ||
            href.includes("/products/") ||
      ...
      const productLinks = withSkuId.length > 0 ? withSkuId : withoutSkuId;
  ...
  async selectVariantsAndGetPrice(
    ...
    // Use endsWith for exact match, plus a broader .includes("EXD-AU") fallback for
    // non-standard suffixes like "-RD-EXD-AU" (e.g. MacBook Mid 2017 SKUs).
    // Also fall back to the product name for products without a recognised SKU suffix.
    const isPristine =
    ...
    // ── CORRECT ORDER: Size → Storage → Color → Condition → Connectivity → Battery → SIM ──
    // Reebelo is nested: condition options only appear after storage+color are set.
    // For Watches: size (MM) must be selected first, before color/condition.

    // 1. Size selection (Watches: 40mm / 44mm / 45mm / 46mm etc.)
    //    throwOnMiss=false — phones and laptops have no size variant, so silently skip.
    ...
    // 2. Storage selection (phones/laptops; Watches have no GB storage — skipped automatically)
    if (specs.storage.length > 0) {
      await this.clickReebeloVariant(page, "storage", specs.storage, "Storage");
    }

    // 3. Color selection (must be before condition)
    if (specs.colors.length > 0) {
      await this.clickReebeloVariant(page, "color", specs.colors, "Color");
    }

    // Wait for page to update after size+storage+color selection
    await randomDelay(1500, 2500);

    // 4. Condition selection (now available after size+storage+color)
    ...
    // 5. Connectivity selection (Watches: Bluetooth / LTE; Phones: Cellular / Wi-Fi)
    //    throwOnMiss=false — single-connectivity products have it pre-selected, no button to click.
  ...
  private static readonly COLOR_ALIASES: ReadonlyMap<string, string[]> =
    new Map([
      ...
      // iPhone 14 Pro colors: Reebelo may label "Space Black" simply as "Black"
      ["space black", ["Black", "Matte Black", "Jet Black"]],
      // "Space Grey" / "Space Gray" fallback to plain "Grey"/"Gray"
      ["space grey", ["Grey", "Gray", "Space Gray"]],
      ["space gray", ["Grey", "Gray", "Space Grey"]],
      // "Deep Purple" fallback to plain "Purple" in case Reebelo omits "Deep"
      ["deep purple", ["Purple"]],
    ]);

  /**
   * Connectivity alias map — normalises product-name terms to Reebelo variant labels.
   * e.g. "Cellular" in the product name → try "LTE", "4G / LTE", "Cellular" on the page.
   * e.g. "Bluetooth" → try "Bluetooth", "BT", "Wi-Fi Only".
   */
  private static readonly CONNECTIVITY_ALIASES: ReadonlyMap<string, string[]> =
    new Map([
      ["bluetooth", ["Bluetooth", "BT", "Wi-Fi Only", "Bluetooth Only"]],
      ["lte", ["LTE", "4G", "4G / LTE", "4G/LTE", "4G LTE", "Cellular"]],
      ["cellular", ["Cellular", "LTE", "4G", "4G / LTE", "4G/LTE", "4G LTE"]],
      ["wi-fi", ["Wi-Fi", "WiFi", "Wi-Fi Only"]],
      ["wifi", ["Wi-Fi", "WiFi", "Wi-Fi Only"]],
      ["4g", ["4G", "LTE", "4G / LTE", "4G/LTE", "Cellular"]],
      ["5g", ["5G", "5G/LTE", "5G NR"]],
    ]);
  ...
  private async clickReebeloVariant(
    ...
    // Build expanded search list with color and connectivity aliases
    ...
    if (variantType === "connectivity") {
      for (const val of targetValues) {
        const aliases = ReebeloSearchService.CONNECTIVITY_ALIASES.get(
          val.toLowerCase(),
        );
        if (aliases) {
          expandedValues.push(...aliases);
