...
export class MobilecitiSearchService {
  ...
  async searchProduct(page: Page, productQuery: string): Promise<AmazonSearchResult[]> {
    try {
      ...
      const isAlreadyOnMobileciti = currentUrl.includes(this.domain);

      if (!isAlreadyOnMobileciti) {
        logger.info(`Visiting Mobileciti homepage...`);
      ...
      // Find search input
      const searchInputSelector = "input#search, input[name='q']";
      ...
      try {
        ...
        await saveDebugSnapshot(page, "no_searchbox_mobileciti");
      ...
      const searchButtonSelector = "button#action-search, button[type='submit']";
      ...
      await page.waitForFunction(() => {
        return (
          !!document.querySelector('.product-item-info, .item.product') ||
          document.body.innerText.includes("Your search returned no results") ||
          document.body.innerText.includes("No results")
        );
      }, { timeout: 15000 }).catch(() => {
        logger.warn("Timeout waiting for Mobileciti search results.");
      });
