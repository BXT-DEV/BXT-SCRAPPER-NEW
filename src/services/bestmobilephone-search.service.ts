...
export class BestmobilephoneSearchService {
  ...
  async searchProduct(page: Page, productQuery: string): Promise<AmazonSearchResult[]> {
    try {
      ...
      const isAlreadyOnBestmobilephone = currentUrl.includes(this.domain);

      if (!isAlreadyOnBestmobilephone) {
        logger.info(`Visiting BestMobilePhone homepage...`);
      ...
      // Find search input
      const searchInputSelector = "form.searchform input.s, input[name='s']";
      ...
      try {
        ...
        await saveDebugSnapshot(page, "no_searchbox_bestmobilephone");
      ...
      await page.waitForFunction(() => {
        return (
          !!document.querySelector('.product-grid-item, .product-item, .item') ||
          document.body.innerText.includes("No products were found") ||
          document.body.innerText.includes("No results")
        );
      }, { timeout: 15000 }).catch(() => {
        logger.warn("Timeout waiting for BestMobilePhone search results.");
      });
    ...
    const results = await page.evaluate((maxResults) => {
      ...
      logger.error(`BestMobilePhone Search failed: ${(error as Error).message}`);
      return [];
    }
  }
}
